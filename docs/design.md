...
- **A subscription has two owning teams, and the rule applies to both.** It joins one team's
  application to another team's product, so the consumer's team and the publisher's team can each
  see it and each end it. That is the same rule, asked twice, rather than an exception to it: what
  the publisher may act on is *the relationship*, which is a thing their product is half of.
  Reaching **inside** it stays consumer-only — a publisher cannot read or rotate somebody else's
  key, because a key rotated by an outsider breaks a caller silently at a moment of that outsider's
  choosing, and because the key is not theirs to learn.
...
A subscription is the one object with **two** owning teams — the team whose application is calling
and the team whose product is being called — and both of them can see it (§2). Neither can see the
other's objects through it: a publisher learns that an application is calling and nothing about the
application itself.
...
**Either side can end a subscription; only the consumer can touch its keys.** Withdrawing access is
a publisher's decision — an abusive or compromised caller is their problem, and needing to find an
administrator to stop it makes the platform the bottleneck at exactly the moment it should not be.
Granting was already theirs, by putting the API in the product. The audit record says which side
did it, because "our key stopped working and nobody here did it" has to be answerable.
