...
Api PUT "/api/resources/$($v1.id)/routes"  @{ environment = "dev"; host = "*"; basePath = "/petstore/v1" } | Out-Null
...
$noKey = Curl "$($Gateways.dev)/petstore/v1/store/inventory"
Check "no api key -> 401" $noKey.Status 401

$badKey = Curl "$($Gateways.dev)/petstore/v1/store/inventory" @("-H", "X-Api-Key: sk_dev_not-a-real-key")
Check "unknown api key -> 401" $badKey.Status 401

$noHeader = Curl "$($Gateways.dev)/petstore/v1/store/inventory" @("-H", "X-Api-Key: $devKey")
Check "failed check-header -> 403" $noHeader.Status 403
Note "body: $($noHeader.Body.Trim())"

$ok = Curl "$($Gateways.dev)/petstore/v1/store/inventory" @(
...
$statuses = @()
foreach ($i in 1..5) {
  $response = Curl "$($Gateways.dev)/petstore/v1/store/inventory" @(
    "-H", "X-Api-Key: $devKey", "-H", "X-Request-Origin: skoda-portal")
...
$otherGateway = Curl "$($Gateways.dev2)/petstore/v1/store/inventory" @(
  "-H", "X-Api-Key: $devKey", "-H", "X-Request-Origin: skoda-portal")
...
foreach ($environment in @("test", "prod")) {
  ...
  Api PUT "/api/resources/$($v1.id)/routes"  @{ environment = $environment; host = "*"; basePath = "/petstore/v1" } | Out-Null
  ...
  $call = Curl "$port/petstore/v1/store/inventory" @(
  ...
  $denied = Curl "$port/petstore/v1/store/inventory" @("-H", "X-Api-Key: $($sub.primaryKey)")
...
$callV1 = Curl "$($Gateways.dev)/petstore/v1/store/inventory" @(
  "-H", "X-Api-Key: $devKey", "-H", "X-Request-Origin: skoda-portal")
$callV2 = Curl "$($Gateways.dev)/petstore/v2/store/inventory" @(
  "-H", "X-Api-Key: $devKey", "-H", "X-Request-Origin: skoda-portal")
...
Start-Sleep -Seconds 3
$deprecated = Curl "$($Gateways.dev)/petstore/v1/store/inventory" @(
  "-H", "X-Api-Key: $devKey", "-H", "X-Request-Origin: skoda-portal")
...
$soapOk = Curl "$($Gateways.dev)/petstore-soap" @(
...
$soapMismatch = Curl "$($Gateways.dev)/petstore-soap" @(
...
$soapNoKey = Curl "$($Gateways.dev)/petstore-soap" @(
  "-X", "POST", "-H", "Content-Type: text/xml", "-H", "SOAPAction: `"urn:apim:petstore:GetPet`"",
...
$badPet = Curl "$($Gateways.dev)/petstore/v2/pet" (@("-X", "POST") + $petHeaders + @("--data", '{"name":42}'))
...
$goodPet = Curl "$($Gateways.dev)/petstore/v2/pet" (@("-X", "POST") + $petHeaders +
...
Start-Sleep -Seconds 3
$warned = Curl "$($Gateways.dev)/petstore/v2/pet" (@("-X", "POST") + $petHeaders + @("--data", '{"name":42}'))
...
Start-Sleep -Seconds 3
$backToBlocking = Curl "$($Gateways.dev)/petstore/v2/pet" (@("-X", "POST") + $petHeaders + @("--data", '{"name":42}'))
...
$served = @()
foreach ($i in 1..4) {
  $call = Curl "$($Gateways.dev)/petstore/v2/store/inventory" @(
    "-H", "X-Api-Key: $devKey", "-H", "X-Request-Origin: skoda-portal")
...
foreach ($i in 1..8) {
  Curl "$($Gateways.dev)/petstore/v2/store/inventory" @(
    "-H", "X-Api-Key: $devKey", "-H", "X-Request-Origin: skoda-portal", "-H", "x-sim-status: 503") | Out-Null
}
$afterBreaker = Curl "$($Gateways.dev)/petstore/v2/store/inventory" @(
  "-H", "X-Api-Key: $devKey", "-H", "X-Request-Origin: skoda-portal")
...
$stillFine = Curl "$($Gateways.dev2)/petstore/v2/store/inventory" @(
  "-H", "X-Api-Key: $devKey", "-H", "X-Request-Origin: skoda-portal")
...
$recovered = Curl "$($Gateways.dev)/petstore/v2/store/inventory" @(
  "-H", "X-Api-Key: $devKey", "-H", "X-Request-Origin: skoda-portal")
...
$preflight = Curl "$($Gateways.dev)/petstore/v1/store/inventory" @(
...
$ownOrigin = Curl "$($Gateways.dev)/petstore/v1/store/inventory" @(
...
Api PUT "/api/resources/$($mcp.id)/routes"  @{ environment = "dev"; host = "*"; basePath = "/petstore-mcp" } | Out-Null
...
$mcpUrl = "$($Gateways.dev)/petstore-mcp"
...
Api PUT "/api/resources/$($a2aResource.id)/routes"  @{ environment = "dev"; host = "*"; basePath = "/shelter" } | Out-Null
...
$card = Curl "$($Gateways.dev)/shelter/.well-known/agent-card.json"
...
$send = Curl "$($Gateways.dev)/shelter" (@("-X", "POST", "-H", "X-Api-Key: $agentKey",
  "-H", "Content-Type: application/json",
...
$stream = Curl "$($Gateways.dev)/shelter" (@("-X", "POST", "-H", "X-Api-Key: $agentKey",
  "-H", "Content-Type: application/json",
...
Start-Sleep -Seconds 3
$events = Curl "$($Gateways.dev)/petstore/v2/events?count=3&intervalMs=100" @(
...
try {
  ...
  $before = Curl "http://localhost:$victimPort/petstore/v1/store/inventory" @(
    "-H", "X-Api-Key: $devKey", "-H", "X-Request-Origin: skoda-portal")
  ...
  $revoked = Curl "http://localhost:$victimPort/petstore/v1/store/inventory" @(
    "-H", "X-Api-Key: $devKey", "-H", "X-Request-Origin: skoda-portal")
  Check "after revocation it refuses traffic -> 503" $revoked.Status 503
  $survivor = Curl "$($Gateways.dev2)/petstore/v1/store/inventory" @(
    "-H", "X-Api-Key: $devKey", "-H", "X-Request-Origin: skoda-portal")
...
Api PUT "/api/resources/$($walk.id)/routes"  @{ environment = "dev"; host = "*"; basePath = "/walkthrough/v1" } | Out-Null
...
Check "the completion panel's address is real" $devRoute.basePath "/walkthrough/v1"
...
Api PUT "/api/resources/$($walk.id)/routes"  @{ environment = "test"; host = "*"; basePath = "/walkthrough/v1" } | Out-Null
...
Start-Sleep -Seconds 3
$consumerCall = Curl "$($Gateways.dev)/walkthrough/v1/store/inventory" @("-H", "X-Api-Key: $walkKey")
...
try {
  ...
  Start-Sleep -Seconds 3
  $beforeAnchor = Curl "$($Gateways.dev)/walkthrough/v1/store/inventory" @("-H", "X-Api-Key: $walkKey")
  ...
  Login "pavel"
  Start-Sleep -Seconds 4
  $afterAnchor = Curl "$($Gateways.dev)/walkthrough/v1/store/inventory" @("-H", "X-Api-Key: $walkKey")
